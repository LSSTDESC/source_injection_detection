...
#======================================
def make_coadd_grid_coord(coadd, mag, stamp_filename, edge=200, step=500): 
    bbox = coadd.getBBox()
    wcs = coadd.getWcs()
    
    x = np.arange(bbox.getMinX()+edge, bbox.getMaxX(), step)
    y = np.arange(bbox.getMinY()+edge, bbox.getMaxY(), step)
    X, Y = np.meshgrid(x,y)
