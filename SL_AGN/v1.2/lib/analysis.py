...
# Histograms of x on top (top-left)
    ax[0, 0].hist(matched[x_col], bins=x_bin_edges, color='b', alpha=0.4, label='True')#, log=True)
    ax[0, 0].hist(unmatched[x_col], bins=x_bin_edges, color='r', alpha=0.4, label='False')#, log=True)
    #ax[0, 0].set_ylabel('Frequency')
    #ax[0, 0].legend()
    ax[0, 0].set_xlim(x_min, x_max)
    ax[0, 0].set_xticks([])
    ax[0, 0].axvline(np.nanmedian(matched[x_col]), ls='--', c='b')
    ax[0, 0].axvline(np.nanmedian(unmatched[x_col]), ls='--', c='r')
    print("line: ", np.nanmedian(matched[x_col]), np.nanmedian(unmatched[x_col]), )
    

# Histograms of y on the right (right-top)
    ax[1, 1].hist(matched[y_col], bins=y_bin_edges, color='b', alpha=0.4, label='True', orientation='horizontal')#, log=True, )
    ax[1, 1].hist(unmatched[y_col], bins=y_bin_edges, color='r', alpha=0.4, label='False', orientation='horizontal')#, log=True, )
    #ax[1, 1].set_xlabel('Frequency')
    #ax[1, 1].legend()
    ax[1, 1].set_ylim(y_min, y_max)
    ax[1, 1].set_yticks([])
    ax[1, 1].axhline(np.nanmedian(matched[y_col]), ls='--', c='b')
    ax[1, 1].axhline(np.nanmedian(unmatched[y_col]), ls='--', c='r')
    print("line: ", np.nanmedian(matched[y_col]), np.nanmedian(unmatched[y_col]), )
